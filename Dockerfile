FROM centos:7
RUN yum install -y python-setuptools git gettext httpd mod_wsgi && easy_install pip && pip install -U django python-gettext && cd /opt && git clone https://github.com/tnaganawa/execjson.git && cd /opt/execjson/web/mysite/ && ./manage.py migrate && ./manage.py compilemessages && chown -R apache.apache /opt/execjson
